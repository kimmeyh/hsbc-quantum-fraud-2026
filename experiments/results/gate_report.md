...
Rows: 157 (120 [SIM], 37 [HW]). Every number below traces to results.json; each cell carries the evidence tag of the rows it summarizes (cvqboost_hw and cvqboost_hw_mixed cells are [HW], all others [SIM]).
...
37 successful fits, 0 failed, metered seconds recorded: 163.0 (None-valued rows: 0)
By arm: cvqboost_hw 27 fits 120.0 s; cvqboost_hw_mixed 10 fits 43.0 s.
