...
1. **Auto-advance window**: between Phase 3.7 approval and the start of Phase 5's team-lead validation, do NOT ask permission at task or phase boundaries; state the next action in one sentence and execute. Before approval, asking is REQUIRED. From validation onward, asking is CORRECT. Commits, pushes, PR-body updates, and test runs are within standing approval.
   - **AND DO NOT END THE TURN (team lead, 2026-09-17, binding).** Inside this window the ONLY thing that ends a turn is arriving at Manual Validation and handing over the recommended steps. Not a completed task, not a completed phase, not a good stopping point, not a progress report. If the next task is executable, execute it in the same turn.
   - **The tell is a closing sentence in the future tense.** "Continuing with Task E", "Next I will convert the scripts", "Moving on to Task F" -- if that sentence is the last thing in the turn, the turn is wrong. Either the work is in the turn or the sentence does not belong in it.
   - **This failed three times in two sprints before it was written down**: Sprint 15 Task C, and Sprint 16 Tasks B and E. Each time a turn ended announcing work that had not started, and the team lead had to ask whether it was actually happening. A rule that lives only in a NOT-do list did not hold; it is an invariant here because this is the document consulted at phase boundaries.
   - **The exceptions are the stopping criteria and nothing else.** `docs/SPRINT_STOPPING_CRITERIA.md` names them: a metered hardware run (Criterion H), a genuine blocker, a decision-class item requiring the team lead. Context or turn length is NOT a stopping reason. If a criterion fires, say WHICH one and why.
