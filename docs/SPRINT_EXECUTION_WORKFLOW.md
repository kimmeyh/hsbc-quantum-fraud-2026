...
- **CI must be GREEN on the pushed HEAD before handover** (`scripts/check_ci_status.py`, exit 0). A local suite pass is not the same claim: Sprint 18's three failures could not fail locally on any run, because they read a branch name that is populated in a normal clone and EMPTY on the detached HEAD `actions/checkout` leaves. The script exits non-zero for red, for still-running AND for cannot-determine, because "I could not tell" is the state that reads as green and is not.
...
- **Re-check CI after the push.** The push is what triggers it, so a Phase 5 check cannot cover a commit made after it.
