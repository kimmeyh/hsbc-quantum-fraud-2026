...
| A5 | Submit via portal: Team Profile + Problem Statement Selection + Concept Proposal PDF + optional supplementary | Guidelines s4 | OK | PORTAL VERIFIED 2026-08-30 (F12, team-lead walkthrough with screenshots). Login works (quantumaiportal.thequantuminsider.com); all five tracks listed, HSBC "Not submitted". SUBMISSION MECHANISM IS FILE UPLOAD, NOT A FORM: the HSBC challenge page has a "YOUR SUBMISSION" panel with drag-and-drop, "0 uploaded, 5 slots left", allowed formats PDF, PNG, JPG, WEBP, GIF, PY, JSON, JS, XLS, XLSX, CSV, DOC, DOCX. Problem-statement selection is implicit (upload lives on the track page). "My files" area tracks files with an "Added to" column. NO team-profile form observed: plan a one-page Team Profile PDF as its own upload slot (see A5b). **SUBMITTED 2026-09-12**, three days before the deadline; portal confirmed "Your submission received." and the counter moved to "3 uploaded, 2 slots left". All four residual unknowns are now ANSWERED (receipt: docs/submission/SUBMISSION_RECEIPT.md): no per-file size cap is displayed; there is NO post-upload metadata prompt; the deadline timezone is not displayed anywhere in the challenge view, so it remains unresolvable from the UI and was made moot by submitting early; and uploading IS the committing action, in two steps -- selecting files stages them with the counter still at 0 and a Remove control per row, and clicking Upload commits. The portal renamed files on ingest (proposal-1, appendix-6, team_profile). |
...
|---|---|---|---|---|---|
| C1 | Problem Framing | Problem Relevance & Impact | 25% | Named statement bottlenecks (detection-experience tradeoff, imbalance, adversarial adaptation). DELIVERED: proposal section 1. NOTE 2026-09-12: this row previously cited "$34bn fraud, $443bn false declines, $4.41 per $1", which the page-limit trim removed. Section 1 now frames the stakes structurally instead -- the fixed analyst review budget, the asymmetric miss-versus-false-alarm cost, and the measured portfolio fraud rates (0.17% ULB, 3.5% IEEE-CIS) -- and cites HSBC's own best-in-class AUC-ROC 0.9459. The requirement is met; the earlier note described text that no longer ships. | OK |
...
| C5 | Validation Plan | Validation Plan | 15% | Metrics, credible benchmarking, success definition; strongest differentiator per guidelines tips. DELIVERED: proposal section 5 (added by F55; this row read "sections 3 and 4" until 2026-09-12, which was the pre-F55 numbering) plus appendix B; gates scored as committed including the failure. | OK |
| C6 | Hybrid Integration | Hybrid / Cross-Domain | 5% | Classical-quantum pipeline rationale. DELIVERED: proposal section 6. (This row read "section 5" until 2026-09-12; F55 inserted the Validation plan as section 5 and pushed Hybrid integration to 6.) | OK |
| C7 | Team Capability | Team Capability | 10% | Expertise, domain knowledge, appropriate composition. NOTE 2026-09-12: this row previously cited a "prior GIC 2026 entry", a phrase that appears in NEITHER shipped document. What the documents actually claim, and what the row now rests on: prior Dirac-3 campaigns on the continuous-variable boosting family including the tuned configuration seeding this submission's quantum arm, quantum ML explainability work, hands-on Amazon Braket, and the Claude Code disclosure. DELIVERED: proposal section 7 and the team profile. The withdrawn GIC 2026 entry was REMOVED at team-lead direction (never submitted). | OK |
...
| D12 | Braket: exec summary "asks" participants to use Amazon Braket; scope allows ANY quantum or quantum-inspired framework; hardware optional, "not penalized" without it, encouraged with it; prototype on SV1/TN1/DM1 first. PORTAL OBJECTIVE (2026-08-30 walkthrough) states it more strongly: "Develop an explainable, deployable quantum or quantum-inspired fraud detection model using Amazon Braket that shows measurable gains over classical baselines" and names the baseline trio (XGBoost, LightGBM, or CatBoost) | s2, 5.3, 5.4 + portal | RESOLVED (team-lead steering 2026-08-30): no Braket execution before submission/acceptance. The proposal covers Braket via (a) the written Phase 2 Braket plan (SV1/DM1 then hardware), tagged [PROJ], and (b) Team Capability citing the team lead's near-expert AWS experience and hands-on Amazon Braket experience, which makes the plan credible without Phase 1 spend. Dirac-3 remains the measured [HW] evidence. The frozen run grid commits no Braket fits, so this is grid-consistent.. Braket named as an UNCONDITIONAL Phase 2 commitment, corrected 2026-09-11 (team lead): the proposal now reads "A Phase 2 Amazon Braket or Classiq arm will run against a matched classical baseline, and will be reported whatever the outcome", and the team-capability section states that hands-on Braket experience "makes a Phase 2 gate-based arm a commitment". The earlier "only if it passes the classical screen" framing was removed from both documents. This row said "gated on a classical feasibility screen" until 2026-09-12 and contradicted the shipped text. | OK |
| D13 | Datasets: IEEE-CIS primary (590k train, 3.5% fraud, up to 393 features), ULB secondary (284,807 rows, 0.172%, 30 features), Sparkov tertiary (1.3M synthetic). Focus on one or two is acceptable | s5.1, 5.4 | ULB shakeout + IEEE-CIS headline; Sparkov [PROJ]. ULB delivered. IEEE-CIS staged and VERIFIED in Sprint 6 (590,540 rows, 434 columns after join, prevalence 3.499% against the stated 3.5%, both source checksums matching MANIFEST.json); loader, preregistered feature pass and rolling-origin splitter built and tested in Sprint 6. Arms RUN in Sprint 8 (F3): 3 duplicates removed, GroupKFold-by-month rolling origin, three classical arms (LightGBM 0.5739, XGBoost 0.5028, CatBoost 0.4795), CVQBoost proxy 0.0571 with a matched-feature control, and the H3 ladder at 12 scoreable cells. Shuffled-label control collapses on every fold. All [SIM]. Sparkov remains [PROJ], which is why this is not OK: the requirement names three datasets and two have delivered. | PARTIAL |
...
| E5 | Under the page limit. MET as of Sprint 12 (F38): proposal 6 of 6, appendix 3 of 3, team profile 1 of 1, measured from the RENDERED PDFs by `scripts/page-fill-report.py`. The xfail(strict=True) markers that tracked this while F38 was open fired the moment the documents came back under limit and have been removed, as their docstring required. Re-verified 2026-09-12 in the Sprint 13 close-out walk. | OK |
| E6 | Reproducibility repo linked as supplementary material. DELIVERED 2026-09-11 (F37): the repository IS public, verified anonymously (HTTP 200; freeze commit 95751b9 resolves; `prereg-freeze` tag intact). Appendix C's claim that every figure regenerates from the public repository is true as written rather than promised. Pre-flip scans across all 259 commits found no secret ever committed. | OK |
