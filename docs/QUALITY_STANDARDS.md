...
**Write edit scripts to a file when they carry LaTeX or regex escapes (Sprint 7 retro improvement 3)**: a heredoc'd Python script containing `\u`, `\s`, `\m` or similar fails with `SyntaxError: (unicode error) 'unicodeescape' codec can't decode bytes` before running a single line. This has cost time in three separate sprints. Write the script to the scratchpad with the Write tool and execute it by path; the escapes survive intact and the failure mode disappears.
