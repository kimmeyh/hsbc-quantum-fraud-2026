...
8. **Task G / F15 (team-lead assigned 2026-08-30): best-practices and ADR review from spamfilter-multi (~3h).** Review its ARCHITECTURE.md and docs/adr/ for (a) architecture/development/SE practices to copy and adapt here, (b) additional SE best-practice suggestions, (c) ML best practices to propose as ADRs (feature engineering, data curation, leakage prevention, training, evaluation), applied to this challenge. Output: docs/adr/ with template, initial ADRs for decisions already made, and a disposition list for the team lead.
9. **Task H / F11 (team-lead-owned): QCi sponsorship letter send + follow-up (~30m).** COMPLETED per team-lead statement 2026-08-30 ("I will handle this one from here. Consider it completed"). Recorded for traceability; no card.
10. **Task I / F12 (team-lead-owned): portal account verification (~15m).** Confirm portal login works; record required submission fields into requirements-matrix A5. Claude records the evidence when reported.
