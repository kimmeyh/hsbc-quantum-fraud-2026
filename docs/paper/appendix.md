...
| Solver draws per fit; identical draws; energy spread | 8; 0 of 27; median 0.019% | [HW] |
...
**Tuned pool (exploratory, A13/A14).** Fit-time class weighting inside each weak learner, the one intervention no earlier control varied. Gram ratio falls 0.9988 to 0.92; L1 from uniform rises to 0.212. Selected on validation AP from four candidates at seed 42, then run over ten seeds: AUPRC 0.7827 (SD 0.0283). Against the single-family pool rebuilt at the SAME k=6 on the SAME splits, the PAIRED difference is +0.0198 (SD 0.0203), positive on 9 of 10 seeds, still BELOW the 0.0268 MDE. The matched comparator matters: quoting the k=13 figure (0.7681) against a k=6 arm is the order-mismatched comparison ADR-0013 warns of. The frozen pool scores 0.7688 at k=6, so the advantage is not a feature-count artifact. Solved minus uniform on the tuned pool is +0.0043: the accuracy came from the learners, not the optimizer.
