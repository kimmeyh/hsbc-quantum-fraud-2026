...
**A.5 IEEE-CIS.** 590,540 transactions, 3 duplicates removed, 3.5% prevalence, rolling-origin evaluation over 30-day TransactionDT buckets, reduced feature recipe after Deotte with UID excluded. Evaluation periods hold 85,302 / 86,524 / 8,111 rows; headline figures are unweighted means and the brackets below are fold min to max across the three folds, not confidence intervals. The shuffled-label control (permuted training labels, true evaluation labels) lands at or below the base rate on every fold; a leaking pipeline would score above it. Hyperparameters are carried from ULB as a hypothesis, so classical figures are floors.
