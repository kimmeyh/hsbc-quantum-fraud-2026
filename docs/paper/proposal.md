...
**The system.** A tuned gradient-boosted ensemble (XGBoost, LightGBM, CatBoost, each given an equal 100-trial Optuna budget) is the production detector. CVQBoost is the evaluated candidate: it trains a pool of weak classifiers on small feature subsets, then selects ensemble weights by minimizing a quadratic objective on the simplex, which Dirac-3 solves natively. The method admits one-, two-, and three-feature subsets; every run reported here used one- and two-feature subsets, giving 91 weak learners over 13 features, because three-feature subsets would require 377 variables and exceed the free-tier device limit available in Phase 1. A third arm, the exact classical solve of the identical objective, serves as a structural control.
