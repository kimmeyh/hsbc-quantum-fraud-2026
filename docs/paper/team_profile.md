...
**Relevant experience.** Prior campaigns on QCi's Dirac-3 using the continuous-variable boosting family, including the tuned configuration that seeds this submission's quantum arm. Quantum machine learning breadth across reservoir computing, phase-based feature engineering, and explainability methods. Near-expert AWS experience with hands-on Amazon Braket, which is why the Phase 2 gate-based arm is a credible plan rather than an aspiration. Applied machine learning on highly imbalanced classification problems of the kind fraud detection presents.
